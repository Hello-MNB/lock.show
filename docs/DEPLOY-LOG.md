...
|---|---|---|---|---|---|---|
| `rel-site-2026.07.21` | 21 Jul 2026 | branch `c73124d` (main UNCHANGED at ef98d91 — shipped by ALIAS-PROMOTION of integration build `lock-site-8hi0ewt9t`, not a merge) | site | **Owner-ordered live update ("תעדכן את האתר לייב").** Desktop content width unified to home's 1100px frame (29 fixes + 3 entity-hero wrappers) · footer Shop link → shop.lock.show w/ utm (footer-only; nav waits for live store) · spotlight-lens logo mark everywhere, DoorStamp deleted · 10 container-contrast fixes (paper↔white, ink↔night) · footer exposed phone+mailto removed per 21 Jul law · zero broken internal links (19/19 pages). **App embed untouched — byte-identical to production ef98d91 (verified by diff before ship).** Note: lock-site project-level ignoreBuildStep cleared during ship attempts (vercel.json copy still governs); my API-created deployments get auto-CANCELED — only git-integration builds succeed, promote by alias. | ✅ tsc + next build 19/19 + eslint + nav-contract 35/35 + team screenshots | live ✓ www.lock.show 200 all pages · shop-link=1 · spotlight-lens=2 · doorstamp=0 · visible phone=0 (+972 only in JSON-LD) · /app 200 unchanged |
