...
|---|---|---|---|---|---|---|
| 🔴 **INCIDENT 2026-07-12** | 12 Jul 2026 | (live state, no deploy) | site | **ALL INNER PAGES 404** on www.lock.show (pricing/faq/artists/bookers/producers/how-it-works/methodology/contact/radar/passport-demo). Homepage 200 ✓, /app/ embed 200 ✓, llms.txt 200 ✓. Files EXIST and serve at `*.html` (pricing.html=200) — extensionless resolution (cleanUrls) is off; apex now 308-redirects to www (domain config changed since 11 Jul verification, likely during accounts work). **FIX READY: branch `hotfix/site-cleanurls` @ d06ac25** (one line: cleanUrls=true in website-next/vercel.json). Claude's main-push blocked by session permissions — Cowork/owner: merge d06ac25 → main, fire site deploy hook, verify /pricing=200. Rollback anchor unchanged @ 917ef57. | playbook §5 | 🔴 broken live — fix staged |
