# LOCK.SHOW — HANDOFF 1 · PRODUCT SPEC (single source)
**Regenerated:** 9 Aug 2026 · from live working sources.
**DoD:** every screen (purpose · canonical object · states · navigation · flows), every flow end-to-end, every entity's operating model, the Radar→Passport bridge, and the Information Architecture (objects · lineage · projections) — a developer designs and wires every screen/flow and implements data meaning without guessing. Open items are ONLY those tagged D2–D6 (Handoff 3).
**Reading order:** §1 Spec → §2 Screen Registry → §3 Flows → §4 Entity Blueprints → §5 Operating Models → §6 Bridge Contract → §7 Information Architecture.
...
## §5A REPRESENTATION OPERATING MODEL
...
## §6 ARTIST BRIDGE CONTRACT (Radar→Passport)
...
| Scene benchmark | System | ⚠ D4 — dataset undecided | Derived | n/a | — | — | Artist-private | Scene (basis line shown) |
...
`platform/source → AI extraction (candidate) → artist confirm/correct/hide → canonical Claim → [private] Radar insight → [approved+policy] Passport fact → share link (one view, one version) → recipient decision/enquiry → Thread → Case → confirmed booking → show outcome → learning-relevant facts refresh Claims`.
