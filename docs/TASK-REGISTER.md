...
**PHASE G — OWNER-GATED UNLOCKS (no build):** G1 = **M-4 counsel sign-off L-1…L-9 (FIRST — blocks ALL real-person processing, not just the scan; urgency: one real user already exists)** · G2 = Anthropic key live-labeling confirmation (key already in the Vercel vault since 8 Jul — needs ONE verified live extraction on a seed artifact, not a build) · G3 = R-11/M-17 (richness, not blockers).
